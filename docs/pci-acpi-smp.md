...
QEMU `-smp 4` on x86/aarch64 (interactive + CI); riscv stays `-smp 2` (Limine panics `missing struct riscv_hart for BSP` at 4). The packed `boot/virt.dtb` must be dumped with the same `-smp` — a single-hart DTB plus OpenSBI BSP hartid=1 reproduces that panic even at `-smp 2`. riscv APs stay parked (like aarch64) so a correct dual-hart DTB does not hang boot after `smp: 2 CPUs online`. On x86, the BSP timer drains UART RX (`input::drain_uart_irq`) so a starved shell under `-smp 4` TCG cannot overrun the COM1 FIFO.
