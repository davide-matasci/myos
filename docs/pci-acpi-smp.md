...
QEMU `-smp 4` on x86/aarch64 (interactive + CI); riscv interactive stays `-smp 2` (Limine panics `missing struct riscv_hart for BSP` at 4) while CI uses `-smp 1` (last green for ripgrep). The packed `boot/virt.dtb` is always dumped with `-smp 2` so interactive OpenSBI BSP hartid=1 still has a hart node; a single-hart DTB reproduces the Limine panic at `-smp 2`. riscv APs stay parked (like aarch64). On x86, post-exec RR re-home is limited to parallel build tools (tcc/make/…) so sequential `/heap` wait stays same-CPU; the BSP timer also drains UART RX (`input::drain_uart_irq`) so a starved shell under `-smp 4` TCG cannot overrun the COM1 FIFO.
