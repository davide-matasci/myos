...
pub fn register(name: &str, bytes: &'static [u8]) -> bool {
    ...
    console::status_progress(&alloc::format!("coreutilsfs full (cannot register {name})"));
    false
}
...
fn log_reg(name: &str, n: usize, replace: bool) {
    ...
    console::status_progress(&alloc::format!("coreutils {kind} {name} ({n} bytes)"));
}

/// Register embedded uutils multicall names generated by `kernel/build.rs`.
pub fn init_embedded() {
    coreutils_embed::register_all();
}

mod coreutils_embed {
    include!(concat!(env!("OUT_DIR"), "/coreutils_embed.rs"));
}
