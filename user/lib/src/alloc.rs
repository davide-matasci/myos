...
/// Page-aligned bump allocation that stays coordinated with [`Heap`].
///
/// Used by `user/tls` for the mbedtls arena so the 2 MiB buffer lives in the
/// brk heap (after the user stack) instead of ELF BSS. A BSS arena was
/// contiguous with the stack: an MPI over-read could walk image→stack→heap
/// and fault at `heap_limit` after corrupting on-stack TLS state (riscv CI).
