...
# --- myos: applied by ports/ripgrep/prepare.sh (not upstream) ---
...
if ! grep -q 'PCRE2_LIB_DIR' "$PCRE2_SYS/build.rs"; then
  cp "$ROOT/ports/ripgrep/pcre2-sys-build.rs" "$PCRE2_SYS/build.rs"
...
if [[ -n "$IGNORE_WALK" ]]; then
  python3 "$ROOT/ports/ripgrep/patch-ignore-myos.py" "$IGNORE_WALK"
fi
