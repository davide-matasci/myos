...
fn poke_ssh_two_clients(key: &Path) -> Result<(), String> {
    ...
    // Overlapping launch: small stagger so the second SYN usually lands after
    // Listen is re-armed; true simultaneous dual-SYN is covered by netd's
    // two-slot parked queue (accepted + accepted_pending).
