...
| `../install-sysroot.sh` | Extract a `.tar.zst` into `target/myos-sysroot` |
| `../check-wire.sh` | Verify `wire-myos.py` applies to pinned nightly |
| `../bump-nightly.sh` | Probe/update pinned nightly when libstd snippets drift |
| `../export-upstream-patch.sh` | Export diff for rust-lang/rust PAL PR |
...
```sh
MYOS_SYSROOT_TARBALL=target/myos-sysroot-<hash>.tar.zst ./toolchain/std/fetch-sysroot.sh
```

Copy `toolchain/std/toolchain/config.toml.example` into your app tree as `.cargo/config.toml`.
...
and all PAL/target files. Re-run `./toolchain/std/build-sysroot.sh` after PAL changes.
