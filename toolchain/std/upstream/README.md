...
| PAL + sys wiring | `toolchain/std/pal/`, `toolchain/std/sys/myos/`, `toolchain/std/os/myos/` | `library/std/src/sys/pal/myos/`, etc. |
| libstd cfg patches | `toolchain/std/patches/wire-myos.py` | Direct edits in `library/std/src/**/mod.rs` |
| Prebuilt sysroot | `toolchain/std/build-sysroot.sh` | Unnecessary once Rust ships target + optional prebuilt std |
...
- `toolchain/std/pal/myos/` → `library/std/src/sys/pal/myos/`
- `toolchain/std/sys/myos/` → `library/std/src/sys/myos/` (+ `alloc/myos.rs`, `fd/myos.rs`, `stdio/myos.rs`)
- `toolchain/std/os/myos/` → `library/std/src/os/myos/`
...
```sh
./toolchain/std/check-wire.sh           # verify patch applies on pinned nightly
./toolchain/std/export-upstream-patch.sh     # -> target/myos-upstream-library.patch
```
...
2. Retain `toolchain/std/package-sysroot.sh` for local installs and CI artifact caching
...
- In-repo: `toolchain/std/pal/README.md`, `toolchain/std/toolchain/config.toml.example`
