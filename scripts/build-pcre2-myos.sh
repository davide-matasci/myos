...
# Sources: target/pcre2-src (fetched). Headers: patches/ripgrep/pcre2-headers
...
"$ROOT/scripts/fetch-pcre2.sh"
"$ROOT/scripts/build-newlib.sh"
export PATH="$ROOT/target/newlib-bin:$PATH"

SRC="$ROOT/target/pcre2-src"
HDR="$ROOT/patches/ripgrep/pcre2-headers"
...
build_arch() {
  ...
  want="$(
    {
      sha256sum "$ROOT/scripts/build-pcre2-myos.sh"
      sha256sum "$ROOT/scripts/fetch-pcre2.sh"
      sha256sum "$ROOT/patches/ripgrep/versions.env"
