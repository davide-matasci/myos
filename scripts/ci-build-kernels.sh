  ...
  h="$(
    (
      cd "$ROOT"
      {
        ...
        # Bump when --print-members grows: old GHCR packages lack hello/ok/OUT_DIR.
        printf 'kernels-oci-members:v2\n'
...
artifacts_ready() {
  # Kernels GHCR must also ship nested hello/ok ELFs (aarch64/riscv host
  # harness) and the myos OUT_DIR (BIOS_PATH/UEFI_PATH are absolute paths
  # baked into target/debug/myos). Master CI #624 failed all boots after a
  # GHCR hit that only restored kernels + target/{bios,uefi}.img.
  [[ -x target/debug/myos ]] \
    && [[ -f target/bios.img ]] \
    && [[ -f target/uefi.img ]] \
    && [[ -f target/aarch64-unknown-none-softfloat/debug/kernel ]] \
    && [[ -f target/riscv64imac-unknown-none-elf/debug/kernel ]] \
    && [[ -f target/hello-aarch64-unknown-none-softfloat ]] \
    && [[ -f target/hello-riscv64imac-unknown-none-elf ]] \
    && [[ -f target/ok-aarch64-unknown-none-softfloat ]] \
    && [[ -f target/ok-riscv64imac-unknown-none-elf ]] \
    && compgen -G 'target/debug/build/myos-*/out/bios.img' >/dev/null
}
...
case "${1:-}" in
  ...
  --print-members)
    ...
    # Nested ELFs copied by kernel/build.rs; aarch64/riscv --ci rebuilds the
    # Limine image from these paths (not only the prebuilt kernel ELF).
    shopt -s nullglob
    for f in target/hello-* target/ok-*; do
      echo "$f"
    done
    # BIOS_PATH/UEFI_PATH point at OUT_DIR, not target/bios.img.
    for d in target/debug/build/myos-*/out; do
      [[ -d "$d" ]] && echo "$d"
    done
    exit 0
    ;;
