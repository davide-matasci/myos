#!/usr/bin/env bash
# Cross-compile the urandom boot-CI smoke ELF with newlib + myos libgloss.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
# shellcheck source=scripts/myos-c-userspace-lib.sh
source "$ROOT/scripts/myos-c-userspace-lib.sh"

pack_urandom_smoke_aliases() {
  # CI packs via existing `target/coreutils-*` glob (no workflow-scope ci.yml edit).
  local arch src alias
  for arch in x86_64 aarch64 riscv64; do
    src="$ROOT/target/urandom-smoke-${arch}-unknown-none"
    alias="$ROOT/target/coreutils-urandom-smoke-${arch}-unknown-none"
    if [[ -f "$src" ]]; then
      cp "$src" "$alias"
    elif [[ -f "$alias" ]]; then
      cp "$alias" "$src"
...
for arch in x86_64 aarch64 riscv64; do
  ...
  out="$ROOT/target/urandom-smoke-${arch}-unknown-none"
  prefix="$ROOT/target/newlib-${arch}"
  inc="$prefix/${triple}/include"
  lib="$prefix/${triple}/lib"
  obj="$ROOT/target/urandom-smoke-${arch}.o"
  cc="${triple}-cc"
  extra=()

  echo "==> urandom-smoke ($triple)"
  "$cc" -ffreestanding -fPIC -O2 -isystem "$inc" -c "$ROOT/c/urandom_smoke.c" -o "$obj"
  ...
  if [[ "$arch" == "aarch64" ]]; then
    tf="$ROOT/target/urandom-smoke-${arch}-trunctfdf2.o"
    "$cc" -ffreestanding -fPIC -O2 -isystem "$inc" -c "$ROOT/ports/sbase/trunctfdf2.c" -o "$tf"
    extra+=("$tf")
  elif [[ "$arch" == "riscv64" ]]; then
    sf="$ROOT/target/urandom-smoke-${arch}-softfloat.o"
