#!/usr/bin/env bash
# Cross-compile the pty boot-CI smoke ELF with newlib + myos libgloss.
set -euo pipefail
...
for arch in x86_64 aarch64 riscv64; do
  ...
  out="$ROOT/target/pty-smoke-${arch}-unknown-none"
  prefix="$ROOT/target/newlib-${arch}"
  inc="$prefix/${triple}/include"
  lib="$prefix/${triple}/lib"
  obj="$ROOT/target/pty-smoke-${arch}.o"
  cc="${triple}-cc"
  extra=()

  echo "==> pty-smoke ($triple)"
  "$cc" -ffreestanding -fPIC -O2 -isystem "$inc" -c "$ROOT/c/pty_smoke.c" -o "$obj"
  ...
  if [[ "$arch" == "aarch64" ]]; then
    tf="$ROOT/target/pty-smoke-${arch}-trunctfdf2.o"
    "$cc" -ffreestanding -fPIC -O2 -isystem "$inc" -c "$ROOT/ports/sbase/trunctfdf2.c" -o "$tf"
    extra+=("$tf")
  elif [[ "$arch" == "riscv64" ]]; then
    sf="$ROOT/target/pty-smoke-${arch}-softfloat.o"
